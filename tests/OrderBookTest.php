...
class OrderBookTest extends TestCase
{
	public function testAdd()
	{
		...
		foreach ($asks as $ask) {
			...
			$orderBook->add(Order::SIDE_ASK, $currencyPair, $rate);
		}

		foreach ($bids as $bid) {
			$rate = new Rate($bid[0], $bid[1]);
			$orderBook->add(Order::SIDE_BID, $currencyPair, $rate);
		}
		...
		$result = $orderBook->remove(Order::SIDE_ASK, $currencyPair, 1.7261);
